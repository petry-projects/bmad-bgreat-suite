...
- Look for existing {ops_artifacts}/`*incident-response*.md`
...
- If no observability plan found: "An observability plan is recommended but not required. Having one helps define alerting triggers for incident detection. You can create one later with the `ops-3-create-observability` workflow."
- Proceed without it
...
Copy the template from `../templates/incident-response-plan-template.md` to `{ops_artifacts}/incident-response.md`
...
- Created: `{ops_artifacts}/incident-response.md` from template
